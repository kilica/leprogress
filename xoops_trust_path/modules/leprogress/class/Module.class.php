...
define('LEPROGRESS_FRAME_PERFORM_SUCCESS', 1);
define('LEPROGRESS_FRAME_PERFORM_FAIL', 2);
define('LEPROGRESS_FRAME_INIT_SUCCESS', 3);

define('LEPROGRESS_FRAME_VIEW_NONE','none');
define('LEPROGRESS_FRAME_VIEW_SUCCESS','success');
define('LEPROGRESS_FRAME_VIEW_ERROR','error');
define('LEPROGRESS_FRAME_VIEW_INDEX','index');
define('LEPROGRESS_FRAME_VIEW_INPUT','input');
define('LEPROGRESS_FRAME_VIEW_PREVIEW','preview');
define('LEPROGRESS_FRAME_VIEW_CANCEL','cancel');

/**
 * Leprogress_Module
**/
class Leprogress_Module extends Legacy_ModuleAdapter
{
	public /*** string ***/ $mActionName = null;

	public /*** Leprogress_AbstractAction ***/ $mAction = null;

	public /*** bool ***/ $mAdminFlag = false;

	public /*** Leprogress_AssetManager ***/ $mAssetManager = null;
	...
	protected /*** Enum[] ***/ $_mAllowViewNames = array(
		LEPROGRESS_FRAME_VIEW_NONE,
		LEPROGRESS_FRAME_VIEW_SUCCESS,
		LEPROGRESS_FRAME_VIEW_ERROR,
		LEPROGRESS_FRAME_VIEW_INDEX,
		LEPROGRESS_FRAME_VIEW_INPUT,
		LEPROGRESS_FRAME_VIEW_PREVIEW,
		LEPROGRESS_FRAME_VIEW_CANCEL
	);
	...
	**/
	public function startup()
	{
		...
		XCube_DelegateUtils::call('Module.leprogress.Global.Event.GetAssetManager',new XCube_Ref($this->mAssetManager),$this->mXoopsModule->get('dirname'));
	...
	**/
	private function _getDefaultActionName()
	{
		...
		$dataname = isset($dataname) ? $dataname : 'Mytask';
	...
	**/
	public function getRenderSystemName()
	{
		...
		if(!defined('LEPROGRESS_ADMIN_RENDER_REGISTED'))
		{
			define('LEPROGRESS_ADMIN_RENDER_REGISTED',true);
			$root =& XCube_Root::getSingleton();
			$root->overrideSiteConfig(
				array(
					'RenderSystems' => array(
						'Leprogress_AdminRenderSystem' => 'Leprogress_AdminRenderSystem'
					),
					'Leprogress_AdminRenderSystem' => array(
						'root' => LEPROGRESS_TRUST_PATH,
						'path' => '/admin/class/LeprogressAdminRenderSystem.class.php',
						'class' => 'Leprogress_AdminRenderSystem'
					)
				)
			);
		}
	
		return 'Leprogress_AdminRenderSystem';
	}
	...
	**/
	public function execute(/*** XCube_Controller ***/ &$controller)
	{
		...
		$viewStatus = (Leprogress_Utils::getEnv('REQUEST_METHOD') == 'POST') ?
	...
	private function _createAction()
	{
		...
		switch(true)
		{
			case file_exists(
				...
				break;
			case file_exists(
				$path = LEPROGRESS_TRUST_PATH . '/' . $fileName
			):
				break;
			default:
				return false;
		...
		$className = 'Leprogress_' . ($this->mAdminFlag ? 'Admin_' : '')
			. ucfirst($this->mActionName) . 'Action';
		if(class_exists($className))
		{
			$this->mAction = new $className();
		}
		if(!$this->mAction instanceof Leprogress_AbstractAction)
		{
			return false;
		}
	...
	**/
	...
	{
		/**
		 * Module.leprogress.Global.Event.Exception.ActionNotFound
		 * 
		 * @param	string	$dirname
		 * 
		 * @return	void
		**/
		XCube_DelegateUtils::call('Module.leprogress.Global.Event.Exception.ActionNotFound',$this->mAssetManager->mDirname);
		/**
	...
	**/
	...
	{
		/**
		 * Module.leprogress.Global.Event.Exception.Preparation
		 * 
		 * @param	string	$dirname
		 * 
		 * @return	void
		**/
		XCube_DelegateUtils::call('Module.leprogress.Global.Event.Exception.Preparation',$this->mAssetManager->mDirname);
		/**
	...
	**/
	...
	{
		XCube_Root::getSingleton()->mController->executeRedirect('./', 1, _MD_LEPROGRESS_ERROR_NO_PERMISSION);

		/**
		 * Module.leprogress.Global.Event.Exception.Permission
		 * 
		 * @param	string	$dirname
		 * 
		 * @return	void
		**/
		//XCube_DelegateUtils::call('Module.leprogress.Global.Event.Exception.Permission',$this->mAssetManager->mDirname);
		/**
